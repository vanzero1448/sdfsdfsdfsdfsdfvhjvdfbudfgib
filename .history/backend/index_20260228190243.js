...
const payLimiter = rateLimit({ windowMs: 60_000, max: 15, message: { error: "Too many requests." } });
const resLimiter = rateLimit({ windowMs: 60_000, max: 60, message: { error: "Too many requests." } });

const REQUIRED_ENV = [
  "ROBOKASSA_MERCHANT_LOGIN", "ROBOKASSA_PASSWORD1", "ROBOKASSA_PASSWORD2",
  "HYRCON_HOST", "HYRCON_PASSWORD", "TELEGRAM_BOT_TOKEN", "TELEGRAM_CHAT_ID", "SITE_URL",
];
...
const VALID_RANKS = new Set(["ping", "packet", "protocol", "script", "kernel", "cipher", "quantum", "core", "overclock", "singularity"]);
...
function buildCommands(itemId, itemType, nick) {
  ...
  if (itemType === "rank") {
    ...
    else if (period === "month") cmds.push(`lp user ${nick} parent addtemp ${rank} 30d`);
    else if (period === "quarter") cmds.push(`lp user ${nick} parent addtemp ${rank} 90d`);
  } else if (itemType === "pack") {
    const cfg = PACK_CONFIG[itemId];
    if (!cfg) return { cmds: [], rankName: null };

    rankName = cfg.rank;
    if (cfg.duration) cmds.push(`lp user ${nick} parent addtemp ${cfg.rank} ${cfg.duration}`);
...
function buildRevokeCommands(p) {
  ...
  if (p.itemType === "pass") return [`lp user ${p.nick} permission unset BetterBattlePass.perks.vip`];
...
const safeCompare = (a, b) => a.length === b.length && crypto.timingSafeEqual(Buffer.from(a), Buffer.from(b));

async function sendRcon(commands) {
  if (!commands.length) return [];
  const rcon = await Rcon.connect({ host: RCON_HOST, port: RCON_PORT, password: RCON_PASS, timeout: 6000 });
  const results = [];
...
async function telegramRequest(method, body) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(
      { hostname: "api.telegram.org", path: `/bot${BOT_TOKEN}/${method}`, method: "POST", headers: { "Content-Type": "application/json", "Content-Length": Buffer.byteLength(data) } },
      (res) => { let raw = ""; res.on("data", (d) => (raw += d)); res.on("end", () => { try { resolve(JSON.parse(raw)); } catch { resolve({}); } }); }
    );
...
function buildPurchaseCaption(p) {
  const status = p.revoked ? "🔴 <b>Status:</b> REVOKED BY ADMIN" : "🟢 <b>Status:</b> ACTIVE";
  return [
  ...
  for (const chatId of ADMIN_CHATS) {
    try {
      const res = await telegramRequest("sendMessage", {
        chat_id: chatId, text: buildPurchaseCaption(p), parse_mode: "HTML",
        reply_markup: p.revoked ? undefined : { inline_keyboard: [[{ text: "🔴 Revoke Privilege", callback_data: `revoke|${p.invId}` }]] },
      });
      if (res?.result?.message_id) sentMessages.push({ chatId, msgId: res.result.message_id });
    } catch (e) { console.warn(`TG send error to ${chatId}:`, e.message); }
  }
  return sentMessages;
}
...
app.post("/create-payment", payLimiter, (req, res) => {
  ...
  if (!nick || !itemId || !itemType || !price) return res.status(400).json({ error: "Missing fields" });
  if (!isValidNick(nick)) return res.status(400).json({ error: "Invalid nickname" });
  if (!isValidSum(price)) return res.status(400).json({ error: "Invalid price" });

  const { cmds } = buildCommands(itemId, itemType, nick);
  if (!cmds.length) return res.status(400).json({ error: "Unknown item or type" });
...
app.post("/robokassa/result", resLimiter, async (req, res) => {
  const { InvId, OutSum, SignatureValue, shp_item, shp_nick, shp_type } = req.body;
  if (!InvId || !OutSum || !SignatureValue || !shp_item || !shp_nick || !shp_type) return res.status(400).send("missing params");

  const expectedSig = `${OutSum}:${InvId}:${PASS2}:shp_item=${shp_item}:shp_nick=${shp_nick}:shp_type=${shp_type}`;
  const mySig = crypto.createHash("md5").update(expectedSig).digest("hex").toUpperCase();

  if (!safeCompare(mySig, (SignatureValue || "").toUpperCase())) return res.status(400).send("bad sign");

  const { cmds, rankName } = buildCommands(shp_item, shp_type, shp_nick);
  try {
    await sendRcon(cmds);
  } catch (e) { console.error(`❌ RCON error:`, e.message); return res.status(500).send("rcon error"); }

  const p = {
    invId: String(InvId), nick: shp_nick, itemId: shp_item, itemType: shp_type,
    itemLabel: buildItemLabel(shp_item, shp_type), price: parseFloat(OutSum).toFixed(2),
    rankName, dateStr: new Date().toLocaleString(), revoked: false, tgMsgs: []
  };
  purchases.set(String(InvId), p);

  try { p.tgMsgs = await sendPurchaseNotification(p); } catch (e) {}
  res.send(`OK${InvId}`);
});
...
app.post("/telegram/webhook", async (req, res) => {
  ...
  await telegramRequest("answerCallbackQuery", { callback_query_id: cbq.id }).catch(() => {});
    ...
    try {
      ...
      for (const msg of p.tgMsgs) {
        await telegramRequest("editMessageText", {
          chat_id: msg.chatId, message_id: msg.msgId, text: buildPurchaseCaption(p), parse_mode: "HTML",
          reply_markup: { inline_keyboard: [[{ text: "✅ Privilege Revoked", callback_data: "noop" }]] },
        }).catch(() => {});
      }
...
app.listen(PORT, async () => {
  ...
  const r = await telegramRequest("setWebhook", { url: `${SITE_URL}/telegram/webhook`, drop_pending_updates: true });
