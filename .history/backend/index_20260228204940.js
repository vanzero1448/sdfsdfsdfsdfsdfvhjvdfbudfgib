...
// │  ВНИМАНИЕ: Старые ID невалидны.                     │
// └─────────────────────────────────────────────────────┘
const EMOJI_IDS = {
  money: "5368324170671202286", // 💸 (НЕВАЛИДЕН)
...
// 🔧 ИСПРАВЛЕНИЕ: Временно возвращаем только fallback (обычные эмодзи)
// Как только соберешь новые ID (поможет логгер ниже), верни строку:
// const tge = (id, fallback) => `<tg-emoji emoji-id="${id}">${fallback}</tg-emoji>`;
const tge = (id, fallback) => fallback;
...
async function sendPurchaseNotification(p) {
  ...
  for (const chatId of ADMIN_CHATS) {
    try {
      const text = buildPurchaseMessage(p);
      console.log(
        `📨  Sending TG notification to ${chatId}, text length: ${text.length}`,
      );
      ...
      if (res?.result?.message_id) {
        ...
        console.log(
          `✅  TG message sent to ${chatId}, msgId: ${res.result.message_id}`,
        );
      } else {
        console.warn(
          `⚠️  TG sendMessage failed for ${chatId}:`,
          JSON.stringify(res),
        );
      }
    } catch (e) {
...
app.post("/telegram/webhook", async (req, res) => {
  ...
  // 🔧 ИСПРАВЛЕНИЕ: Логгер для получения новых ID эмодзи
  const msg = req.body?.message;
  if (msg && msg.entities) {
    const customEmojis = msg.entities.filter((e) => e.type === "custom_emoji");
    if (customEmojis.length > 0) {
      console.log("\n👀 НОВЫЙ CUSTOM EMOJI ID ПОЛУЧЕН:");
      customEmojis.forEach((e) => {
        const text = msg.text
          ? msg.text.substring(e.offset, e.offset + e.length)
          : "Эмодзи";
        console.log(`Символ: ${text} -> ID: ${e.custom_emoji_id}`);
      });
      console.log("Добавьте этот ID в объект EMOJI_IDS!\n");
    }
  }
