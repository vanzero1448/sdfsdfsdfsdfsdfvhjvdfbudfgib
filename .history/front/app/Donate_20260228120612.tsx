...
export const Donate: React.FC = () => {
  ...
  useEffect(() => {
    if (rankModal) {
      // Небольшая задержка, чтобы DOM успел отрендериться перед расчетом позиций
  ...
  // Открытие модалок со сбросом стейта
  ...
  // Валидация никнейма (Защита от инъекций и спецсимволов)
  const validateNickname = (name: string) => {
    if (!name.trim()) return "Nickname is required";
    // Регулярное выражение Minecraft/Hytale: только латиница, цифры и _, от 3 до 16 символов
    if (!/^[a-zA-Z0-9_]{3,16}$/.test(name))
      return "Invalid format (3-16 chars, a-z, 0-9, _)";
    return "";
  };

  // Универсальный обработчик покупки (Готов к подключению Robokassa)
  const handlePurchase = async (
    ...
    try {
      // ИМИТАЦИЯ ЗАПРОСА К СЕРВЕРУ (В следующем шаге мы заменим это на fetch к Render)
      console.log(
        `Preparing purchase: ${itemType} [${itemId}] for ${nick} ($${price})`,
      );

      await new Promise((resolve) => setTimeout(resolve, 1500)); // Фейковая задержка 1.5с

      // Временное сообщение об успехе подготовки
      alert(
        `Backend integration ready! In the next step, this will redirect ${nick} to Robokassa to pay $${price} for ${itemId}.`,
      );
    } catch (err) {
      console.error(err);
  ...
  return (
    <>
      <style>{`
/* ВЕСЬ ТВОЙ CSS СОХРАНЕН БЕЗ ИЗМЕНЕНИЙ */
...
.sp-card:hover .sp-cover{transform:scale(1.04)}
...
.nick-lbl{font-family:var(--fm);font-size:9px;font-weight:600;letter-spacing:.13em;
  text-transform:uppercase;color:var(--t3);margin-bottom:7px;display:flex;justify-content:space-between}
...
.nick-inp.invalid{border-color: rgba(239, 68, 68, 0.5);}
...
.m-buy:disabled{opacity:0.5; cursor:not-allowed}

.pm-body{display:grid;grid-template-columns:1fr 220px}
@media(max-width:580px){.pm-body{grid-template-columns:1fr}}
.pm-right{border-left:1px solid var(--w);display:flex;flex-direction:column;
  align-items:stretch;background:var(--s2);border-radius:0 18px 18px 0;
  overflow:hidden;position:sticky;top:0;align-self:start;}
.pm-kit-zone{position:relative;width:100%;aspect-ratio:1/1;overflow:hidden;
  background:var(--s3);border-bottom:1px solid var(--w);flex-shrink:0;}
.pm-kit-glow{position:absolute;inset:0;opacity:.2;pointer-events:none;z-index:1;}
.pm-kit-img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;
  object-position:center;z-index:2;transition:transform .5s cubic-bezier(.25,.46,.45,.94);}
.pm-kit-img:hover{transform:scale(1.05)}
.pm-kit-label{position:absolute;bottom:10px;left:50%;transform:translateX(-50%);
  font-family:var(--fm);font-size:8px;font-weight:600;letter-spacing:.14em;
  text-transform:uppercase;padding:3px 10px;border-radius:4px;border:1px solid;
  z-index:3;white-space:nowrap;background:rgba(0,0,0,.5);backdrop-filter:blur(6px);}
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="m-body">
              <div className="m-left">
                ...
                >
                  <Zap size={14} />
                  {loading ? "Processing..." : "Purchase Battle Pass · $7"}
                </button>
              </div>
              <div className="m-right">
                <img
